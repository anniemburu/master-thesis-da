...
for config in "${CONFIGS[@]}"; do
    ...
    python3 bin_finder.py --config "$config" --model_name MLP --binning sturges --strategy uniform > output_test.log 2>&1
    python3 bin_finder.py --config "$config" --model_name MLP --binning freedman --strategy uniform > output_test.log 2>&1
